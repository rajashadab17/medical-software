...
} from "../../../Data/Female_Height_Age";
Chart.register(CategoryScale);

const Female_Height_Graph = ({height, width, legends, sentRemarks, Report, Report_Year}) => {
...
const Height_Array = Report_Year ? Reports_Data.filter(val => val.Report_Year <= Report_Year).map((val) => val.Height_Centimeter) : Reports_Data.map((val) => val.Height_Centimeter)
  ...
  // const Height_Array = Reports_Data.map((val) => val.Height_Centimeter);
  ...
  setTimeout(() => {
    ...
    // if (lastPointIndexNumber > P_10) {
    // // if (Number(lastPointIndexNumber) < Number(P_5)) {
    //   console.log({ condition: lastPointIndexNumber > P_5, lastPointIndexNumber, P_5})
    //   setRemarks("Normal");
    // } else if (lastPointIndexNumber >= P_5 && lastPointIndexNumber <= P_10) {
    //   setRemarks("B/L Stunted");
    // } else if (lastPointIndexNumber < P_5) {
    //   setRemarks("Stunted");
    // }  else {
...
export default Female_Height_Graph;
